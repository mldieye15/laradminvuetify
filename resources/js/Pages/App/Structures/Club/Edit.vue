<template >
    <AppLayout>
        ...
        <v-card>
            <v-card-title>
                <span class="text-h5">Edition club</span>
            </v-card-title>
            <v-card-text>
                <v-container>
                    <v-row>
...
export default {
    ...
    props:  ['club', 'ligueRegionales', 'errors'],
    data () {
      return {
        searchLigue: '',
        items: [
    ...
    created () {
        this.form = Object.assign({}, this.club[0]);
        this.selectedLigue = JSON.parse(this.form.ligue_regionale);
    },

    methods: {
        save () {
            Inertia.post(`/app/fede/structures/clubs/${this.form.id}`, {
                _method: 'put',
