<template >
    <AppLayout>
        ...
        <v-card>
            <v-card-title>
                <span class="text-h5">Edition association</span>
            </v-card-title>
            <v-card-text>
                <v-container>
                    <v-row>
...
export default {
    ...
    props:  ['association', 'ligueRegionales', 'errors'],
    data () {
      return {
        searchLigue: '',
        items: [
            ...
            },
            {
                text: 'Edition',
                disabled: true,
                route: 'ligreassociationgio.edit',
            }
        ],
    ...
    created () {
        this.form = Object.assign({}, this.association[0]);
        this.selectedLigue = JSON.parse(this.form.ligue_regionale);
    },

    methods: {
        save () {
            Inertia.post(`/app/fede/structures/associations/${this.form.id}`, {
                _method: 'put',
